...
class CertificateServiceConfigAssembler:
    """
    Assembles the Certificate Service config file
    """
    ...
    def __init__(self, aws_region, environment):
        ...
        self.__secrets_client = boto3.client(
            "secretsmanager", region_name=self.__region
        )
    ...
    def __get_secret(self, secret_arn) -> str:
        ...
        try:
            ...
            print(
                f"Error retrieving secret: {e.response['Error']['Code']} - {secret_arn}"
            )
            return ""
    ...
    def write_config(file_handle, key, value) -> None:
        ...
        """
        entry = (
            f"{key}={CertificateServiceConfigAssembler.escape_env_value(str(value))}\n"
        )
        file_handle.write(entry)

    def construct_config_file(self) -> None:
        """
        Constructs the config file for Certificate Service
        ...
        certificate_queue = os.environ.get(
            "CERTIFICATE_QUEUE", f"{stage}-{self.__project_name}-certificate-queue.fifo"
        )
        registrations_table = f"{stage}-{self.__project_name}-registrations"
        events_table = f"{stage}-{self.__project_name}"
        s3_bucket = f"{stage}-{self.__project_name}-file-bucket"
        ...
        if self.__input_environment == "test" or stage == "test":
            ...
            frontend_url = self.__get_parameter(
                f"{parameter_store_prefix}/frontend-url-{self.__stage}", decrypt=False
            )
            ses_smtp_username = self.__get_parameter(
                f"{self.__project_name}-techtix-smtp-username", decrypt=True
            )
            ses_smtp_password = self.__get_parameter(
                f"{self.__project_name}-techtix-smtp-password", decrypt=True
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Certificate Service Configuration Assembler"
    )
    ...
    config_assembler = CertificateServiceConfigAssembler(region, input_stage)
